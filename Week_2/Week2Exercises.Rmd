...
Here I called the stringr library and then used colnames() to make the columns into a vector. I then pulled the first indice and renamed it to Row.ID.
...
Here I took difference between the time it takes to ship by subtracting the order date by the ship date and then took the average of that result. After that I divided it by ddays(1) to get the duration in days.
...
Here I used str_which to find the indices of the word 'table' in the Product.Name column of sales. I then used length to count the total number of occurrences. 
